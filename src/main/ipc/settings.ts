...
export function registerSettingsHandlers(
  ...
  ipcMain.handle('settings:exportPortable', async (event): Promise<SettingsExportResult> => {
    try {
      const keybindingSnapshot = keybindings?.getSnapshot()
      const document = createSettingsExportDocument(store.getSettings(), {
        keybindings: keybindingSnapshot
          ? {
              keybindings: keybindingSnapshot.commonOverrides,
              platforms: keybindingSnapshot.platformOverrides
            }
          : undefined
      })
  ...
  ipcMain.handle(
    ...
    async (event): Promise<SettingsImportPreview> => {
      ...
      try {
        ...
        return { ...previewSettingsExportImport(parsed, store.getSettings()), filePath }
      } catch (error) {
        return {
          ok: false,
          ...emptyPreview,
