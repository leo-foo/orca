...
import { sanitizeFloatingWorkspaceDirectorySetting } from '../ipc/floating-workspace-directory'
...
// to a Set once at module load lets the per-key membership check stay O(1)
// without re-coercing the readonly tuple on every call.
...
export type SettingsUpdateApplier = (
  args: Partial<GlobalSettings>,
  originWebContentsId?: number
) => Promise<GlobalSettings>

/** The single application path for settings updates: sanitizes the payload,
 *  persists it, and runs every main-process side effect (theme, menus, proxy,
 *  app icon, telemetry). Used by settings IPC and portable import. */
export function createSettingsUpdateApplier(
  store: Store,
  agentAwakeService?: AgentAwakeService
): SettingsUpdateApplier {
  return async (args, originWebContentsId) => {
